...
在C++中，this指针作为一个隐藏参数传递给所有非静态成员函数调用。它的类型取决于函数声明。如果将类X的成员函数声明为const，则其类型为const X*（请参见下面的代码1），如果将成员函数声明为volatile，则其类型为volatile X*（请参见下面的代码2），如果成员函数声明为const volatile，则其类型为const volatile X*（请参见下面的代码3）。
