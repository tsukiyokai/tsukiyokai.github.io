...
每当我们为一个类定义一个或多个非默认构造函数（有参）时，也应显式定义一个默认构造函数（无参），因为在这种情况下编译器将不提供默认构造函数。虽然这不是必须的，但是始终定义默认构造函数一般被认为是最佳实践。
...
1. 当类的对象按值返回时；
2. 当类的一个对象被值作为参数传递给函数时；
3. 当一个对象基于同一类的另一个对象来构造时；
4. 当编译器生成临时对象时。

然而，在所有这些情况下，都不能保证拷贝构造函数一定被调用，因为C++标准允许编译器在某些情况下优化拷贝。
