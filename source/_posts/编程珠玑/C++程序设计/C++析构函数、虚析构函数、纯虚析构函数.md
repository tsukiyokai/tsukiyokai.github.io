...
子类的析构函数其实不止只有析构子类的这些代码，C++语言隐式生成了很多其他代码，即析构顺序，在调用析构函数时会先调用子类的析构函数，然后反向依次析构子类的成员变量，然后再反向析构各基类，如果有多个基类，按照先非virtual基类后virtual基类的顺序。并且子类的析构函数不是立刻返回的，而是等到上述函数全部调用完才返回。
