...
---
虚函数是运行时多态，不是编译时。
在继承机制中，规定：基类指针可以指向派生类对象。（把男女老少当人，把金毛泰迪当狗）
虚函数这里，需要注意的是使用基类指针调用派生类的函数。
其思想是，根据指向或引用的对象实例的类型调用虚拟函数，而不是根据指针或引用的类型。
...
为了达成初衷，只需要改写基类中的getArea函数如下：
`virtual void get_Area() { cout << "调用基类Shape的求面积函数" << endl; }`
...
2.vptr：指向vtable的指针，按对象实例维护（参见[此](https://www.geeksforgeeks.org/c-virtual-functions-question-12/)示例）。
...
结果：a>b
分析：A类有一个在B类中不存在的VPTR。在虚拟函数的典型实现中，编译器为每个对象放置一个VPTR。编译器秘密地在每个构造函数中添加一些代码。
...
默认参数不参与函数签名。因此，基类和派生类中fun()的签名被认为是相同的，因此基类的fun()被重写。另外，默认值在编译时使用。当编译器发现函数调用中缺少参数时，它将替换给定的默认值。因此，在上述程序中，x的值在编译时被替换，并在运行时调用派生类的fun()。
...
请按任意键继续. . .
```
原因跟上一个相同，默认值在编译时被替换。fun()在基类型的指针bp上调用。所以编译器将其替换为0（不是10）。
...
想起来去年接触的一句话：数学公式里有pi的地方背后一定藏着一个圈，即使这个公式和圆看起来八竿子打不着。
...
例如，以下程序将打印“C::fun() called”，因为B::fun()虽然没写virtual，但实质上已经自动变为虚了，所以自B派生出的C自然也是虚的fun()。
...
上述代码有编译错误。
分析一下：
错误1：仅非静态成员函数可以是虚拟的（要去掉virtual）
错误2：静态成员函数上不允许使用类型限定符（要去掉const）

## 虚析构函数

使用具有非虚析构函数的基类类型指针删除派生类对象会导致未定义的行为。为了纠正这种情况，应该使用虚拟析构函数来定义基类。
...
在C++中，RTTI（运行时类型信息）是一种在运行时公开有关对象数据类型的信息的机制，并且仅对具有至少一个虚函数的类可用。它允许在程序执行期间确定对象的类型。
...
例如，让Shape为基类。我们无法在Shape中提供功能draw()的实现，但是我们知道每个派生类都必须具有draw()的实现。同样，动物类也没有move()的实现（假设所有动物都在移动），但是所有动物都必须知道如何移动。我们不能创建抽象类的对象。（抽象类不能实例化）
...
最后再趁热打铁复习一下以前背过的C++八股里的虚函数部分，加深一下理解。

#### 为什么析构函数总是设为虚函数？如果这是必要的，那么为什么C++不把虚析构函数直接作为默认值？

将可能会被继承的基类的析构函数设置为虚函数，使用基类指针指向其子类对象，当释放基类指针时可以释放掉子类的空间，防止内存泄漏。

C++默认的析构函数不是虚函数是因为虚函数需要额外的虚函数表和虚表指针，占用额外的内存。而对于不会被继承的类来说，其析构函数如果是虚函数，就会浪费内存。因此C++默认的析构函数不是虚函数，而是只有当需要当作基类时，设置为虚函数。
