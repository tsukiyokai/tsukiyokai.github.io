...
本来只写了5行代码就通过了常规的测试用例。结果提交的时候在各种畸形用例上翻车。翻车的两个用例："a " 和 " a "，连续翻车了14次，第15次才完全通过。代码也在不知不觉间扩充到接近30行了。想一次性考虑到各种极端情况还是挺困难的，应多注意锻炼这方面的能力。Think twice, code once. 并不是那么容易做到。
