...
我提交的第一个版本，prev 表示上一级的数字，如要计算 AAA，则 prev 就是 ZZ 的数值。diff 是 prev 与目标数字的差，即(prev, AAA]。
...
每一位的权重是 26：ans = ans \* 26 + num;

从左到右遍历，其实连续乘以 26 并保存下来就行，不是非得要只乘一次 pow(26, len-i)，脑子不要这么耿直。
