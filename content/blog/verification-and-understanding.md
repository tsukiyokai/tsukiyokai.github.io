+++
title = "形式之外：论代码验证与文档审阅的存在论差异"
date  = 2026-04-03
description = "从代码验证与文档审阅之间的非对称性出发，考察形式语言与自然语言在意义存在方式上的存在论断裂。"
[taxonomies]
...
形式语言通过人为地截断延异的运动来制造确定性。在一个类型系统中，`String`的含义不依赖于使用它的程序员的理解，不随语境变化，不指向系统外的任何东西。这是通过将"含义"重新定义为"形式规则所规定的行为"来实现的，本质上是一种约定性的暴力：它通过排除延异来使验证成为可能。
